...
def autism_predictor():
    ...
    if request.method == "POST":
        try:
            ...
            # --- START: DATABASE INSERTION LOGIC (NEW) ---
            try:
                ...
                cursor = conn.cursor()
                # Note: The 'family_asd' column is not in the form, so it's not included here.
                # The 'id' column will auto-increment in the database.
                sql = "INSERT INTO autism_predictions (name, age, sex, ethnicity, result) VALUES (%s, %s, %s, %s, %s)"
                values = (name, age, sex, ethnicity, label)
                
                cursor.execute(sql, values)
                conn.commit()
                
                print(f"✅ Data for '{name}' inserted successfully into the database.")

            except mysql.connector.Error as err:
                # Log the error but don't stop the user from seeing their result
